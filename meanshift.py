...
class MeanShift(object):
    ...
    def shift_points(self,center_point,whole_points,Gaussian_sigma):
        ...
        for each_point in whole_points:#遍历每一个点
            ...
            #所有向量相加
            shifting_px += Gaussian_weight * (each_point[0] - center_point[0])
            shifting_py += Gaussian_weight * (each_point[1] - center_point[1])
            sum_weight += Gaussian_weight
        #归一化
        shifting_px /= sum_weight
        shifting_py /= sum_weight
        return [shifting_px + center_point[0],shifting_py + center_point[1]]
...
def main():
    ...
    shifted_points, mean_shift_result = mean_shifter.fit(X, Gaussian_sigma=0.4)#Gaussian核设置为0.5,对X进行mean_shift

    np.set_printoptions(precision=3)
    print('input: {}'.format(X))
    print('assigned clusters: {}'.format(mean_shift_result))
